...
object SimpleStreamTransducers {
  ...
  sealed trait Process[I,O] {
    ...
    def my_|>[O2](p2: Process[O,O2]): Process[I,O2] = {
      this match {
        case Halt() => Halt()
        case Await(recv) => Await[I, O2](recv(_) |> p2)
        case e@Emit(h: O, t: Process[I, O]) => p2 match {
          case Halt() => Halt()
          case Await(recv2) => recv2(Some(h)) match {
            case Await(recv3) => t |> recv3(Some(h))
            case p => e |> p
          }
  ...
  object Process {
    ...
    def drop[I](n: Int): Process[I,I] = Await[I,I] {
      case Some(i) => if (n>0) drop(n-1) else emit(i)
      case _ => Halt()
    }.repeat
    ...
    def dropWhile[I](f: I => Boolean): Process[I,I] = Await[I,I] {
      case Some(i) => if (f(i)) dropWhile(f) else emit(i, lift(identity))
      case _ => Halt()
    }.repeat
    ...
    def count[I]: Process[I,Int] = {
      def go(acc: Int): Process[I,Int] = await[I, Int](_ => emit(acc+1, go(acc+1)))
      go(0)
    }
    ...
    def mean: Process[Double,Double] = {
      def go(count: Int, acc: Double): Process[Double,Double] = {
        await(d => emit((acc+d)/(count+1), go(count+1, acc+d)))
      }
      go(0,0.0)
    }
    ...
    def my_exists[I](f: I => Boolean): Process[I,Boolean] = await[I, Boolean](i => Emit(f(i))).repeat
...
object GeneralizedStreamTransducers {
  ...
  object Process {
    ...
    def eval[F[_],A](a: F[A]): Process[F,A] =
      await[F,A,A](a) {
        case Right(r) => Emit[F, A](r, Halt(End))
