...
trait Monad[M[_]] extends Functor[M] {
  ...
  def sequence[A](lma: List[M[A]]): M[List[A]] = lma.foldLeft(unit(List.empty[A]))(map2(_, _)(_ :+ _))
...
object Monad {
  ...
  def stateMonad[S] = {
    ...
    new Monad[St] {
      ...
      override def flatMap[A, B](ma: St[A])(f: A => St[B]): St[B] = ma.flatMap(f)
    }
  }

  val idMonad: Monad[Id] = null//???
