...
object Par {
  ...
  def map2WithTimeout[A,B,C](a: Par[A], b: Par[B], timeout: Long, timeUnit: TimeUnit)(f: (A,B) => C): Par[C] = {
    (es: ExecutorService) => {
      ...
      UnitFuture{
        ...
        val b = bf.get(timeUnit.convert(timeout, TimeUnit.MILLISECONDS) - (endA - startA), TimeUnit.MILLISECONDS)
        f(a, b)
      }
    }
  }
  ...
  def parFilter[A](as: List[A])(f: A => Boolean): Par[List[A]] =
    as.foldRight(Par.unit(List.empty[A]))((a, parAcc) => {
      Par.map2(unit(a), parAcc)((a2, acc) => if (f(a2)) a2 :: acc else acc)
    })
  ...
  def chooser[A,B](pa: Par[A])(choices: A => Par[B]): Par[B] = choiceGeneral(pa)(choices)

  //these are the same as xxxWithGeneral
  def choiceWithChooser[A](cond: Par[Boolean])(t: Par[A], f: Par[A]): Par[A] =
    chooser(cond){ bool => es => if (bool) t(es) else f(es) }
  def choiceNWithChooser[A](n: Par[Int])(choices: List[Par[A]]): Par[A] =
    chooser(n){ n => run(_)(choices(n)) }
