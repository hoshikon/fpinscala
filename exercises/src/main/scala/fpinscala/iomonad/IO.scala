...
object IO3 {
  ...
  */
  ...
  def runTrampoline[A](a: Free[Function0,A]): A = a match {
    ...
    case FlatMap(s, f) => s match {
      ...
      case FlatMap(s2, f2) => runTrampoline(s2.flatMap(a => f2(a).flatMap(f)))
    }
  }
  ...
  def run[F[_],A](a: Free[F,A])(implicit F: Monad[F]): F[A] = step(a) match {
    ...
    case Suspend(s) => s
    case FlatMap(s, f) => s match {
      case Suspend(s) => F.flatMap(s)(a => run(f(a))(F))
      case _ => sys.error("Impossible, since `step` eliminates these cases")
    }
  }
  ...
  def step[F[_],A](a: Free[F,A]): Free[F,A] = a match {
    ...
    case FlatMap(FlatMap(s, f1), f2) => step(s.flatMap(a => f1(a).flatMap(f2)))
    case _ => a
  }
  ...
  */
  ...
  def read(file: AsynchronousFileChannel,
           fromPosition: Long,
           numBytes: Int): Par[Either[Throwable, Array[Byte]]] = Par.async { (cb: Either[Throwable, Array[Byte]] => Unit) =>
